...
function getCollectionListCacheKey(query?: ICollectionFilterQuery) {
  ...
  const { collection_type_id, grade_id, scale_id, release_type_id, sort, limit, offset } = query;
  const releaseTypeKey =
    release_type_id && release_type_id.length > 0
      ? [...release_type_id].sort((a, b) => a - b).join('-')
      : undefined;
  const hasQuery =
    [collection_type_id, grade_id, scale_id, limit, offset].some(
  ...
  return `collection_list_ct${collection_type_id ?? 'all'}_g${grade_id ?? 'all'}_sc${scale_id ?? 'all'}_l${limit ?? 'all'}_o${offset ?? 'all'}_s${sort ?? 'default'}_rt${releaseTypeKey ?? 'none'}`;
}
